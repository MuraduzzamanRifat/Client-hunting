...
def init_db():
    ...
    try:
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_sendlog_date ON send_log(sent_at);
            CREATE INDEX IF NOT EXISTS idx_sendlog_emailid ON send_log(email_id, sent_at);
        """)
...
def get_all_emails_for_sync():
    ...
    conn = get_db()
    try:
        return conn.execute("SELECT * FROM emails ORDER BY collected_at ASC").fetchall()
    finally:
        conn.close()
